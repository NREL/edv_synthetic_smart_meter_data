...
puts "Found #{totalcount} xml files in this directory. "

puts "###############################################"
puts "Adding epw file path/name and ddy file path/name in the control file."
i=0
Dir.glob("#{root_dir}/*.xml").each do |xml_file|
  i=i+1
  matches = epw_arr.select {|row| row[:building_id] === File.basename(xml_file, ".xml") }
  
  puts "Matching building #{i} from #{totalcount} buildings: #{matches}"
