...
class HourlyConsumptionByFuelToCSV_Test < MiniTest::Test
  ...
  def test_number_of_arguments_and_argument_names
    ...
    measure = HourlyConsumptionByFuelToCSV.new
  ...
  def test_building_1
    ...
    test_name = "test_building_1"
    csv_path = "#{run_dir(test_name)}/report.csv"
    ...
    # make sure the report file exists "BuildingName.csv"
    assert(File.exist?(csv_path))
  end

  # todo - figure how how to test with ideal air loads (2) vs. (2b) which doesn't use ideal air loads.
  def test_building_2

    test_name = "test_building_2"
    csv_path = "#{run_dir(test_name)}/report.csv"
    model_in_path = "#{File.dirname(__FILE__)}/0524_b.osm"
    epw = OpenStudio::Path.new(File.dirname(__FILE__)) / OpenStudio::Path.new('USA_CO_Golden-NREL.724666_TMY3.epw')
